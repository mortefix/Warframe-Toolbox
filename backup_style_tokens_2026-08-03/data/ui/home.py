...
WATERMARK_PT = 30
SCROLLBAR_H = 12          # matches the QScrollBar:horizontal height in qss.py
...
class Card(QFrame):
    ...
    The whole card is the click target, not just the button - the button sits
    in the far corner, so the text is the natural place to aim.
    """
    ...
    def mouseReleaseEvent(self, event) -> None:
        if self._cmd and event.button() == Qt.LeftButton:
            self._cmd()
