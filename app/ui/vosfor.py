...
class ProgressBar(QFrame):
    ...
    def __init__(self, fraction: float, complete: bool) -> None:
        ...
        self._colour = t.OK_SURFACE if complete else t.ACCENT_SURFACE
...
class ArcaneRow(QWidget):
    """One arcane: completion mark, count, then a wiki look-up and the name.

    The wiki glyph sits directly LEFT of the name (right of the fixed-width
    count + completion mark), so every row's look-up icon still lines up in one
    vertical column. It is the row's ONLY interactive element - the row itself
    no longer toggles anything, because owned/not-owned is read from the
    inventory, not set by hand."""
    ...
    def __init__(self, arc: dict) -> None:
        ...
        # Wiki look-up, directly left of the name. The mark and count before it
        # are both fixed width, so the icons still align on the y-axis. t.WIKI_ICON
        # is a table KEY, not something a font can draw: passing it straight to a
        # label rendered the letters "wiki" in the icon face (oddly wide link).
