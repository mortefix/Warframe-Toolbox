...
_text_loaded: bool | None = None


def ensure_text_fonts() -> bool:
    """Register the bundled themeable TEXT faces (every other .ttf in
    assets/fonts) so a theme's `core.theme.FONT_FACE` can select them by family.

    Idempotent, and needs a running QApplication like `ensure_loaded`. Missing
    or unreadable files are simply skipped: a theme that names an absent family
    falls back through `resolve_family` to the system chain, so the app still
    runs - it just draws that category in Segoe rather than the intended face.
    """
    global _text_loaded
    if _text_loaded is not None:
        return _text_loaded
    if QApplication.instance() is None:
        return False
    ok = False
    for ttf in sorted(FONT_PATH.parent.glob("*.ttf")):
        if ttf == FONT_PATH:                 # Material Symbols is ensure_loaded's
            continue                         # job (and already registered)
...
def clear_cache() -> None:
    """Drop the rendered-icon cache. Icons are keyed by colour, so stale entries
    are harmless, but a live theme switch clears them so the map does not grow an
    entry per theme visited and freshly built widgets tint from the new palette."""
    _CACHE.clear()
